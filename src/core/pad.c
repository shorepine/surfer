...
static struct {
    uint8_t  dpad;          /* SURF_DPAD_* bitmask */
    uint16_t buttons;       /* SURF_BTN_* bitmask */
    int32_t  stick[2][2];   /* [stick][axis] Q16 in [-SURF_ONE, SURF_ONE] */
} g_pads[SURF_MAX_PADS];

static bool pad_ok(int p) { return p >= 0 && p < SURF_MAX_PADS; }

uint8_t surf_pad_dpad(int p) { return pad_ok(p) ? g_pads[p].dpad : 0; }
uint16_t surf_pad_buttons(int p) { return pad_ok(p) ? g_pads[p].buttons : 0; }
...
void surf_pad_set_dpad(int p, uint8_t d)
{
    if (pad_ok(p))
        g_pads[p].dpad = d & 0x0f;
}

void surf_pad_set_buttons(int p, uint16_t b)
{
    if (pad_ok(p))
        g_pads[p].buttons = b;
}
